...
void interrupts::idt_gate(uint64_t IRQ)
{
...
void interrupts::idt_expection(uint64_t IRQ)
{
...
void interrupts::mask_irq(unsigned char channel)
{
    uint16_t data;
...
void interrupts::clear_irq(unsigned char channel)
{
    uint16_t data;
...
void interrupts::call_handler(uint64_t irqNum)
{
    irqFuncs[irqNum - 1]();
}

extern "C" void irq_handler(uint64_t irqNum)
{
    interrupt_controller.call_handler(irqNum);
}
