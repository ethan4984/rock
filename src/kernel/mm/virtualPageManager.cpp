...
struct mappingIndexes {
    mappingIndexes(uint64_t addr, uint64_t *pml4) {
...
void mapping::copy(mapping m1) {
    ...
    for(int i = 0; i < 0x200; i++) {
        if(m1.pml4[i] & 0b1) {
            pml4[i] = pmm::calloc(1) | GET_PMLX_FLAGS(m1.pml4[i]);
            ...
            for(int j = 0; j < 0x200; j++) {
                if(m1pml3[j] & 0b1 && !(m1pml3[j] & (1 << 7))) {
                    pml3[j] = pmm::calloc(1) | GET_PMLX_FLAGS(m1pml3[j]);
                    ...
                    for(int k = 0; k < 0x200; k++) {
                        if(m1pml2[k] & 0b1 && !(m1pml2[k] & (1 << 7))) {
                            pml2[k] = pmm::calloc(1) | GET_PMLX_FLAGS(m1pml2[k]);
...
void mapping::mapRange(uint64_t virtualBase, uint64_t cnt, uint64_t extraFlags) {
    uint64_t physicalBase = pmm::alloc(cnt);
    
    for(size_t i = 0; i < cnt * 0x1000; i += 0x1000) {
        map(pml4, physicalBase + i, virtualBase + i, 0x3 | extraFlags, 0x3 | extraFlags);
    }
}
...
void mapping::setup() {
    pml4 = (uint64_t*)(pmm::calloc(1) + HIGH_VMA);
    copy(mappings[0]);
}
...
void map(uint64_t *pml4, uint64_t physicalAddr, uint64_t virtualAddr, uint64_t flags, uint64_t flags1) {
    mappingIndexes indexes(virtualAddr, pml4);

    if(indexes.pml3 == NULL) {
        indexes.pml3 = (uint64_t*)(pmm::calloc(1) + HIGH_VMA);
        pml4[indexes.pml4idx] = ((uint64_t)indexes.pml3 - HIGH_VMA) | flags; 
    }

    if(indexes.pml2 == NULL) {
        indexes.pml2 = (uint64_t*)(pmm::calloc(1) + HIGH_VMA);
        indexes.pml3[indexes.pml3idx] = ((uint64_t)indexes.pml2 - HIGH_VMA) | flags; 
    }

    if(!(flags1 & (1 << 7))) { // check for 4kb pages
        if(indexes.pml1 == NULL) {
            indexes.pml1 = (uint64_t*)(pmm::calloc(1) + HIGH_VMA);
...
void unmap(uint64_t *pml4, uint64_t virtualAddr, uint64_t flags) {
    mappingIndexes indexes(virtualAddr, pml4);
...
void init() {
    uint64_t *pml4 = (uint64_t*)(pmm::calloc(1) + HIGH_VMA);
    uint64_t *pml3 = (uint64_t*)(pmm::calloc(1) + HIGH_VMA);

    uint64_t *pml3_HH = (uint64_t*)(pmm::calloc(1) + HIGH_VMA);
    uint64_t *pml2_HH = (uint64_t*)(pmm::calloc(1) + HIGH_VMA);

    uint64_t *pml2_1G = (uint64_t*)(pmm::calloc(1) + HIGH_VMA);
