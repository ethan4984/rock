...
struct vector {
    void (*handler)(regs*, void*);
    void *ptr;
    bool reserved;
};

static vector interrupt_vector[256];
...
int alloc_vec(void (*f)(regs*, void*), void *ptr) {
    for(size_t i = 0; i < 256; i++) {
        if(!interrupt_vector[i].reserved) {
            interrupt_vector[i].ptr = ptr;
            interrupt_vector[i].handler = f;
            interrupt_vector[i].reserved = true;
            return i;
        }
    }
    return -1;
...
void stacktrace(regs *regs_cur) {
    size_t *rbp = (size_t*)regs_cur->rbp;

    for(;;) {
        size_t old_rbp = *rbp++;
        size_t return_address = *rbp;
        ...
        bool symbol = [&]() {
            for(size_t i = 0;; i++) {
                if(symlist[i].addr >= return_address) {
                    offset = return_address - symlist[i - 1].addr;
                    name = symlist[i - 1].name;
                    return true; 
                }
            }
            return false;
...
extern "C" void isr_handler_main(regs *regs_cur) {
    if(regs_cur->isr_number < 32) {
        ...
        print("[KDEBUG] Kowalski analysis: \"{}\", Error: {x}, LAPIC_ID {x}\n", exception_messages[regs_cur->isr_number], regs_cur->err_code, apic::lapic->read(apic::lapic->id_reg()));
    ...
    interrupt_vector[regs_cur->isr_number].handler(regs_cur, interrupt_vector[regs_cur->isr_number].ptr);
...
void idt_init() {
    ...
    for(size_t i = 0; i < 256; i++) {
        interrupt_vector[i].handler = disr;
        interrupt_vector[i].ptr = NULL;
    }

    interrupt_vector[32] = (vector) {   sched::reschedule,
                                        NULL,
                                        true
                                    };

    interrupt_vector[33] = (vector) {   tty::ps2_keyboard,
                                        NULL,
                                        true
                                    };
