...
void init() {
    madt_ptr = find_SDT<madt>("APIC");

    for(size_t i = 0; i < madt_ptr->acpihdr_ptr.length - (sizeof(madt_ptr->acpihdr_ptr) + sizeof(madt_ptr->lapic_addr) + sizeof(madt_ptr->flags)); i++) {
