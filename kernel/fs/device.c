...
static_vec(devfs_node_t, devfs_nodes);

static filesystem_t devfs = {   .mount_gate = "/dev",
                                .read = ramfs_read,
                                .write = ramfs_write,
                                .open = ramfs_open,
                                .unlink = ramfs_delete
...
int devfs_add_device(devfs_node_t node) {
    vfs_node_t *devfs = vfs_absolute_path("/dev");
    if(devfs == NULL)
        return -1;

    node.devfs_id = devfs_id_cnt++;
    vec_push(devfs_node_t, devfs_nodes, node);

    vfs_node_t *new_dev = vfs_create_node(devfs, node.dev_name);
    return new_dev->fs->open(new_dev, DEVFS_PERMS);
}

devfs_node_t *path2devfs(char *path) {
    vfs_node_t *device = vfs_absolute_path(path);
    if(device == NULL)
        return NULL;

    for(size_t i = 0; i < devfs_nodes.element_cnt; i++) {
        devfs_node_t *node = vec_search(devfs_node_t, devfs_nodes, i);
...
static int device_check_fs(devfs_node_t *dev) {
...
void scan_device_partitions(msd_t *device) {
    ...
    if(mbr_signature == 0xaa55) { 
        ...
        for(int i = 0; i < 4; i++) {
            ...
            devfs_node_t devfs_node = { .dev_name = kmalloc(DEV_NAME_MAX),
            ...
            devfs_node_t *new_devfs_node = path2devfs(dev_absolute_path);
    ...
    device->read(device->device_index, SECTOR_SIZE, sizeof(gpt_partition_table_hdr), &gpt_hdr);
