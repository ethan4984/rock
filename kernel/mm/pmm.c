...
void pmm_init(stivale_t *stivale) {
    ...
    uint64_t bm_size = total_mem / 0x1000 / 8;
    ...
    kprintf("[KMM]", "Total detected memory: %x", total_mem);
}

uint64_t pmm_alloc(uint64_t cnt) {
    uint64_t base = bm_first_free() * 0x1000, count = 0;
    for(uint64_t i = bm_first_free(); i < total_mem / 0x1000; i++) {
        if(BM_TEST(bitmap, i)) {
            base += (count + 1) * 0x1000;
            count = 0;
            continue;
...
static int bm_first_free() { 
    for(uint64_t i = 0; i < total_mem / 0x1000; i++) {
        if(!BM_TEST(bitmap, i)) {
            return i;
        }
    }
