...
int main(void) {
	print("DUFAY: VFS: booting server\n");
	...
	print("DUFAY: VFS: Slab cache directory initialised\n");

	constexpr int NOTIFICATION_STACK_SIZE = 0x10000;
	for(int i = 0; i < 4; i++) {
		uintptr_t addr;
		int ret = as_allocate(&address_space, &addr, NOTIFICATION_STACK_SIZE);
		if(ret == -1) { print("DUFAY: VFS: Failed to allocate address for stack\n"); goto failure; }
		...
		if(response.ret == -1) print("DUFAY: VFS: failed to allocate notification stack\n");
		else print("DUFAY: VFS: Allocated notificaton stack #%d [%x:%x]\n",
			i, addr, NOTIFICATION_STACK_SIZE);
	}

	uintptr_t addr;
	int ret = as_allocate(&address_space, &addr, 0x10000);
	if(ret == -1) { print("DUFAY: VFS: Failed to allocate address\n"); goto failure; }

	ret = vfs();
	if(ret == -1) { print("DUFAY: VFS: Internal critical failure\n"); goto failure; }
failure:
	for(;;);
}
